...
export default class Pull {
  constructor(el, options) {
    ...
    this.enabled = true;
    this.canfresh = false;
    this.msgWrapStyle = children[0][0].style;
    this.contentWrapStyle = children[1][0].style;
  ...
  [_move](e) {
    ...
    if (isTop) {
      ...
      if (diffY > this.options.distance * 1.5) {
        this.options.MsgElement(this.options.msgs[0]);
        this.canfresh = true;        
      } else {
        this.options.MsgElement(this.options.msgs[1]);
        this.canfresh = false;
      }
